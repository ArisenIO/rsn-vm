...
#define RSN_VM_OPEN_NAMESPACE namespace arisen::vm {
#define RSN_VM_CLOSE_NAMESPACE }

#include "error_codes_pp.hpp"

// create a category for parser error codes
GENERATE_ERROR_CATEGORY(parser_errors, "rsn-vm parser errors")
GENERATE_ERROR_CATEGORY(memory_errors, "rsn-vm memory errors")
GENERATE_ERROR_CATEGORY(system_errors, "rsn-vm system errors")
GENERATE_ERROR_CATEGORY(auxiliary_errors, "rsn-vm auxiliary errors")
...
#define _RSN_VM_ASSERT(expr, err_type)                                                                                  \
   if (!UNLIKELY(expr)) {                                                                                              \
      return err_type;                                                                                                 \
   }

// assert but invalidate the enclosing class
#define RSN_VM_ASSERT_INVALIDATE(expr, err_type)                                                                       \
