...
#include <arisen/vm/guarded_ptr.hpp>
      ...
      public:
	    ...
	    bytes_used = 0;
         ...
         inline constexpr void from( guarded_ptr<uint8_t>& code ) {
            uint8_t cnt = 0;
            for (;; cnt++) {
               RSN_VM_ASSERT( cnt < bytes_needed<N>(), wasm_interpreter_exception, "incorrect leb128 encoding" );
               RSN_VM_ASSERT( code.offset()+cnt < code.bounds(), wasm_interpreter_exception, "pointer out of bounds" );
               ...
               if ((storage[cnt] & 0x80) == 0) {
                  if( cnt + 1 == bytes_needed<N>() ) {
                     ...
                     RSN_VM_ASSERT((mask & storage[cnt]) == 0, wasm_parse_exception, "unused bits of unsigned leb128 must be 0");
                  }
                  break;
               }
            }
               ...
               if ((storage[cnt] & 0x80) == 0) {
                  if( cnt + 1 == bytes_needed<N>() ) {
                    uint32_t offset = N - 7*(bytes_needed<N>()-1);
                     ...
                     RSN_VM_ASSERT((mask & storage[cnt]) == expected, wasm_parse_exception, "unused bits of signed leb128 must be the same as the sign bit");
                  }
                  break;
               }
            }
