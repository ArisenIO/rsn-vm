...
/* clang-format off */
#define RSN_VM_CONTROL_FLOW_OPS(opcode_macro)   \
...
#define RSN_VM_BR_TABLE_OP(opcode_macro)        \
   opcode_macro(br_table, 0x0E)
#define RSN_VM_RETURN_OP(opcode_macro)          \
   opcode_macro(return_, 0x0F)
#define RSN_VM_CALL_OPS(opcode_macro)           \
   opcode_macro(call, 0x10)                     \
   opcode_macro(call_indirect, 0x11)            \
   opcode_macro(br_table_data, 0x12)
#define RSN_VM_CALL_IMM_OPS(opcode_macro)       \
...
#define RSN_VM_PARAMETRIC_OPS(opcode_macro)     \
...
#define RSN_VM_VARIABLE_ACCESS_OPS(opcode_macro)\
...
#define RSN_VM_MEMORY_OPS(opcode_macro)         \
...
#define RSN_VM_I32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i32_const, 0x41)
#define RSN_VM_I64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i64_const, 0x42)
#define RSN_VM_F32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f32_const, 0x43)
#define RSN_VM_F64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f64_const, 0x44)
#define RSN_VM_COMPARISON_OPS(opcode_macro)     \
...
#define RSN_VM_NUMERIC_OPS(opcode_macro)        \
...
#define RSN_VM_CONVERSION_OPS(opcode_macro)     \
...
#define RSN_VM_EXIT_OP(opcode_macro)            \
   opcode_macro(exit, 0xC0)
#define RSN_VM_EMPTY_OPS(opcode_macro)          \
...
#define RSN_VM_ERROR_OPS(opcode_macro)          \
   opcode_macro(error, 0xFF)

/* clang-format on */

#define RSN_VM_CREATE_ENUM(name, code) name = code,

#define RSN_VM_CREATE_STRINGS(name, code) #name,

#define RSN_VM_CREATE_MAP(name, code) { code, #name },

#define RSN_VM_OPCODE_NAME_if_
#define RSN_VM_OPCODE_NAME_else_
#define RSN_VM_OPCODE_NAME_return_

#define RSN_VM_OPCODE_NAME_TEST() 1
#define RSN_VM_OPCODE_NAME_TEST_RSN_VM_OPCODE_NAME_TEST 0,
#define RSN_VM_OPCODE_NAME_TEST_1 1, ignore
#define RSN_VM_EXPAND(x) x
#define RSN_VM_CAT2(x, y) x ## y
#define RSN_VM_CAT(x, y) RSN_VM_CAT2(x, y)
#define RSN_VM_APPLY(f, args) f args
#define RSN_VM_FIX_OPCODE_NAME_0(name) name ## _t
#define RSN_VM_FIX_OPCODE_NAME_1(name) name ## t
#define RSN_VM_FIX_OPCODE_NAME(iskeyword, garbage) RSN_VM_FIX_OPCODE_NAME_ ## iskeyword

#define RSN_VM_OPCODE_T(name)                                                             \
   RSN_VM_APPLY(RSN_VM_FIX_OPCODE_NAME,                                                   \
      (RSN_VM_CAT(RSN_VM_OPCODE_NAME_TEST_,                                               \
           RSN_VM_EXPAND(RSN_VM_OPCODE_NAME_TEST RSN_VM_OPCODE_NAME_ ## name ()))))(name)

#define RSN_VM_CREATE_EXIT_TYPE(name, code)                                                                      \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t pc;                                                                                                     \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define RSN_VM_CREATE_CONTROL_FLOW_TYPES(name, code)                                                                   \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() {}                                                                                       \
      RSN_VM_OPCODE_T(name)(uint32_t data) : data(data) {}                                                             \
      RSN_VM_OPCODE_T(name)(uint32_t d, uint32_t pc, uint16_t i, uint16_t oi)                                          \
...
#define RSN_VM_CREATE_BR_TABLE_TYPE(name, code)                                                                        \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
...
#define RSN_VM_CREATE_TYPES(name, code)                                                                                \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define RSN_VM_CREATE_CALL_TYPES(name, code)                                                                           \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
...
#define RSN_VM_CREATE_CALL_IMM_TYPES(name, code)                                                                       \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      uint16_t locals;                                                                                                 \
...
#define RSN_VM_CREATE_VARIABLE_ACCESS_TYPES(name, code)                                                                \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
...
#define RSN_VM_CREATE_MEMORY_TYPES(name, code)                                                                         \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
...
#define RSN_VM_CREATE_I32_CONSTANT_TYPE(name, code)                                                                    \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
      explicit RSN_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit RSN_VM_OPCODE_T(name)(int32_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         int32_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define RSN_VM_CREATE_I64_CONSTANT_TYPE(name, code)                                                                    \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
      explicit RSN_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit RSN_VM_OPCODE_T(name)(int64_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         int64_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define RSN_VM_CREATE_F32_CONSTANT_TYPE(name, code)                                                                    \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
      explicit RSN_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit RSN_VM_OPCODE_T(name)(float n) { data.f = n; }                                                          \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         float    f;                                                                                                   \
      } data;                                                                                                          \
...
#define RSN_VM_CREATE_F64_CONSTANT_TYPE(name, code)                                                                    \
   struct RSN_VM_OPCODE_T(name) {                                                                                      \
      RSN_VM_OPCODE_T(name)() = default;                                                                               \
      explicit RSN_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit RSN_VM_OPCODE_T(name)(double n) { data.f = n; }                                                         \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         double   f;                                                                                                   \
      } data;                                                                                                          \
...
#define RSN_VM_IDENTITY(name, code) arisen::vm::RSN_VM_OPCODE_T(name),
#define RSN_VM_IDENTITY_END(name, code) arisen::vm::RSN_VM_OPCODE_T(name)
