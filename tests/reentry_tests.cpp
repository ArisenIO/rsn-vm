...
BACKEND_TEST_CASE("test reentry", "[reentry]") {
   ...
   struct test_runner {
      arisen::vm::backend<test_runner, TestType>& bkend;
      ...
      void arisen_assert(uint32_t, uint32_t) {}
      void* memset(void*, int, int) { return 0; }
   };

   using rhf_t     = arisen::vm::registered_host_functions<test_runner>;
   using backend_t = arisen::vm::backend<test_runner, TestType>;
   wasm_allocator wa;
   ...
   rhf_t::template add<test_runner, &test_runner::arisen_assert, wasm_allocator>("env", "arisen_assert");
