...
TEST_CASE("Testing signals", "[invoke_with_signal_handler]") {
   bool okay = false;
   try {
      arisen::vm::invoke_with_signal_handler([]() {
         std::raise(SIGSEGV);
      }, [](int sig) {
...
TEST_CASE("Testing throw", "[signal_handler_throw]") {
   CHECK_THROWS_AS(arisen::vm::invoke_with_signal_handler([](){
      arisen::vm::throw_<arisen::vm::wasm_exit_exception>( "Exiting" );
   }, [](int){}), arisen::vm::wasm_exit_exception);
}
...
TEST_CASE("Test signal handler forwarding", "[signal_handler_forward]") {
   ...
   auto guard = arisen::vm::scope_guard{[]{
      std::signal(SIGSEGV, SIG_DFL);
      std::signal(SIGBUS, SIG_DFL);
      std::signal(SIGFPE, SIG_DFL);
      arisen::vm::setup_signal_handler_impl(); // This is normally only called once
   }};
   {
      std::signal(SIGSEGV, &handle_signal);
      std::signal(SIGBUS, &handle_signal);
      std::signal(SIGFPE, &handle_signal);
      arisen::vm::setup_signal_handler_impl();
      sig_handled = 0;
      std::raise(SIGSEGV);
